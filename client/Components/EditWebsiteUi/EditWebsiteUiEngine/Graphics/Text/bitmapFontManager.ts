...
export class BitmapFontManager {
    ...
    public static onMessage(message: Message): void {
        ...
        if (message.code === MESSAGE_ASSET_LOADER_ASSET_LOADED + `http://localhost:9000/EditorResources/Fonts/fonts.json`) {
            Message.unsubscribeCallback(MESSAGE_ASSET_LOADER_ASSET_LOADED + `http://localhost:9000/EditorResources/Fonts/fonts.json`, BitmapFontManager.onMessage);
    ...
    public static load(): void {
        ...
        const asset = AssetManager.getAsset('http://localhost:9000/EditorResources/Fonts/fonts.json');
        if (asset !== undefined) {
            BitmapFontManager.processFontAsset(asset as JsonAsset);
        } else {
            // Listen for the asset load.
            Message.subscribeCallback(MESSAGE_ASSET_LOADER_ASSET_LOADED + 'http://localhost:9000/EditorResources/Fonts/fonts.json', BitmapFontManager.onMessage);
        }
    }
